...
int recv_packet(unsigned char *paquet, struct lws *wsi)
{
	...
	*/
	...
	{
		case 0x10 :
			update(paquet); // gene poour voir si le bleu est en mvt
...
fonction ramene
      		...
      		Cell* cible =recherche_bot(chaine);
      		while (cible!=NULL){
				if blue dans liste de vision chaine alors 
					test si coord actual == coord dans liste du bleu // ne pas uodate sinon pas de diff entre les coord act et prece
						create bool "follow trajectoire bleu" a true
						if bool follow ==true
							decal selon le coeff directeur 
						if bool follow == true && mout dans vision devant // set as id cible and keep
							maneeuvre du jaune autour du mout
								dedans un bool "manoeuvre"==true
							bool follow = false
						if manoeuvre==true 
							placement dehind
								bool pousse =true
						...
						if pousse == true
							while distance mout / jaune >50 && pas angle trop grand && distance centre / mout > 900
								pointeur = mout
							if angle trop grand
								placement behind
							if distance centre / mout > 900
								supressID(chaine, cible->nodeID);
						      	cible=recherche_bot(chaine);
